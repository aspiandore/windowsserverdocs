...
| /systore | Set the system storage device (this is only valid for EFI systems; the setting is not retained if a reboot is performed and is used only in cases where the system storage device is ambiguous). |
...
| /mirror | Create mirror entries in the archive. |
...
#### Parameters that control remote event logging

| Parameter | Description |
| ------ | ----------- |
| /eventsettings | Set the global parameters for remote event logging. |
| /event | Enable or disable remote event logging for an operating system entry. |
